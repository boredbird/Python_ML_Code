# -*- coding: utf-8 -*-
"""
Created on Fri Dec 08 15:49:40 2017
...
def _bestSplit(binDS, method, BinNo):
                ...
                newbinDS_0.loc[:,'i'] = range(1, len(newbinDS_0)+1)
                newbinDS_1.loc[:,'i'] = range(1, len(newbinDS_1)+1)
...
def _combineBins(temp_cont, target):
                ...
                for i in temp_cont.index:
                    ...
                    if i == temp_cont.index.max():
                        aix = temp_cont.loc[temp_cont.index<i,:].index.max()
                    else:
                        aix = temp_cont.loc[temp_cont.index>i,:].index.min()
                    
                    if any(rowdata[:3] == 0):#如果0,1,total有一项为0，则运行
                        
                        temp_cont.loc[aix, target] = temp_cont.loc[aix, target] + rowdata[target]
                        temp_cont.loc[aix, 0] = temp_cont.loc[aix, 0] + rowdata[0]
                        temp_cont.loc[aix, 'total'] = temp_cont.loc[aix, 'total'] + rowdata['total']
                        #
                        if i < temp_cont.index.max():
                            temp_cont.loc[aix,'lower'] = rowdata['lower']
                        else:
                            temp_cont.loc[aix,'upper'] = rowdata['upper']
...
def binContVar(x, y, method, mmax=5, Acc=0.01, target=1, adjust=0.0001):
                ...
                # 下面对一句是我根据结果不一致的情况做的修改 2017年12月11日
                t.index=t.index+1
                print 't',t
...
def ju_applyBinMap(df,x, bin_map):
                """
                Generate result of bining by bin_map
                ------------------------------------------------
                Params
