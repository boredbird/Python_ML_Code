# -*- coding: utf-8 -*-
"""
Created on Tue Nov 28 14:50:19 2017
                ...
                # Info value
                    ...
                    if any([_isNullZero(N_mat[i][0]),
                        ...
                        for j in range(int(R)):
                            IV = IV + (N_mat[j][0]/N_s[0] - N_mat[j][1]/N_s[1])\
                                *np.log((N_mat[j][0]*N_s[1])/(N_mat[j][1]*N_s[0]))
                        M_value = IV
...
def _EqualWidthBinMap(x, Acc, adjust):
                ...
                Mbins = int(1./Acc)
                minMaxSize = float((varMax - varMin)/Mbins)
...
def a_binContVar(x, y, method, mmax=5, Acc=0.01, target=1, adjust=0.0001):
                """
                Optimal binings for contiouns var x by (y & method)
                method is represent by number, 
                    1:Gini, 2:Entropy, 3:person chisq, 4:Info value
